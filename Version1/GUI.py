...
from db_control_sql import get_filtered_hotels, make_reservation, cancel_reservation
...
class HotelApp:
    ...
    def setup_main_window(self):
        ...
        # Ana menüde iptal butonu ekleyin
    ...
    def complete_reservation(self):
        try:
            ...
            if success != -1:
                ...
                f"Your reservation code: {success}\n\n"
                "IMPORTANT: Please save this code. "
                "You will need it if you want to cancel your reservation later."
                )
                messagebox.showinfo("Reservation Successful", message)
                self.reservation_window.destroy()
                if self.hotels_window:
                    self.hotels_window.destroy()
            else:
    ...
    def open_cancellation_window(self):
        ...
        if self.cancellation_window is not None:
            return

        self.cancellation_window = tk.Toplevel(self.root)
        self.cancellation_window.title("Cancel Reservation")
        self.cancellation_window.geometry("300x200")

        # def on_closing():
        #     self.cancellation_window = None
        #     self.cancellation_window.destroy()

        # self.cancellation_window.protocol("WM_DELETE_WINDOW", on_closing)
        # Reservation code entry
        
        tk.Label(self.cancellation_window, text="Enter Reservation Code:").pack(pady=10)
        self.reservation_code_entry = tk.Entry(self.cancellation_window)
        self.reservation_code_entry.pack(pady=5)

        # Guest ID entry
        tk.Label(self.cancellation_window, text="Enter Guest ID:").pack(pady=10)
        self.guest_id_entry = tk.Entry(self.cancellation_window)
        ...
        self.cancel_button = tk.Button(
            ...
            text="Cancel Reservation",
            command=self.handle_cancellation
        )
        self.cancel_button.pack(pady=20)

    def handle_cancellation(self):
        try:
            reservation_code = self.reservation_code_entry.get().strip()
            guest_id = self.guest_id_entry.get().strip()

            if not reservation_code or not guest_id:
                messagebox.showerror("Error", "Please enter both the reservation code and guest ID.")
                return

            result = cancel_reservation(reservation_code, guest_id)
            if result['success']:
            ...
            messagebox.showerror("Error", "Please enter a valid reservation code and guest ID.")
