...
def validate_guest_and_reservation(guest_tc, reservation_id):
    """Validate that the provided Guest TC and Reservation ID are valid and linked."""
    conn = None
    try:
        conn = sqlite3.connect('HotelManagement.db', timeout=20)
        cursor = conn.cursor()
        ...
        query = """
            SELECT g.guest_id, r.hotel_id
            FROM reservations r
            JOIN guests g ON r.guest_id = g.guest_id
            WHERE g.guest_tc = ? AND r.reservation_id = ?
        """
        
        cursor.execute(query, (guest_tc, reservation_id))
        result = cursor.fetchone()
        return result  # Returns (guest_id, hotel_id) if valid, otherwise None

    except sqlite3.Error as e:
        print(f"Database error in validate_guest_and_reservation: {e}")
        return None
...
def add_comment_to_db(content, num_stars, guest_id, hotel_id):
    ...
    try:
        ...
        query = """
            INSERT INTO comments (date, content, num_stars, guest_id, hotel_id)
            VALUES (CURRENT_DATE, ?, ?, ?, ?)
        """
        
        cursor.execute(query, (content, num_stars, guest_id, hotel_id))
        conn.commit()
        return True

    except sqlite3.Error as e:
        print(f"Database error in add_comment_to_db: {e}")
        return False
