...
class HotelApp:
    ...
    def main_menu(self):
        ...
        tk.Button(self.root, text="Add Comment", width=20, command=self.add_comment).pack(pady=10)
    
    def show_comment_form(self, parent_window, reservation_info):
        """Display the form for adding a comment."""
        # Destroy the validation window
        for widget in parent_window.winfo_children():
            widget.destroy()

        tk.Label(parent_window, text="Add Your Comment", font=("Arial", 16)).pack(pady=10)

        # Comment text area
        tk.Label(parent_window, text="Your Comment:").pack(pady=5)
        comment_text = tk.Text(parent_window, height=8, width=50)
        comment_text.pack(pady=5)

        # Rating selection
        tk.Label(parent_window, text="Rating (1-5):").pack(pady=5)
        ...
        def submit_comment():
            ...
            success = self.save_comment(
                hotel_name=reservation_info["hotel_name"],
                guest_tc=reservation_info["guest_tc"],
                comment=comment,
                rating=rating,
            )
        ...
        tk.Button(parent_window, text="Submit", command=submit_comment).pack(pady=10)

    def save_comment(self, hotel_name, guest_tc, comment, rating):
        """Function to save comment to the database (this needs to be implemented)."""
        try:
            # Assuming you have a function to interact with the database
            # This is where you would interact with your database to save the comment
            # For example:
            result = make_comment_in_db(hotel_name, guest_tc, comment, rating)
            
            return result  # return True if success, False if failure
    ...
    def add_comment(self):
        ...
        comment_window.geometry("500x300")

        tk.Label(comment_window, text="Add a Comment", font=("Arial", 16)).pack(pady=10)

        # Guest TC entry
        tk.Label(comment_window, text="Guest TC:").pack(pady=5)
        guest_tc_entry = tk.Entry(comment_window)
        guest_tc_entry.pack(pady=5)

        # Reservation ID entry
        tk.Label(comment_window, text="Reservation ID:").pack(pady=5)
        reservation_id_entry = tk.Entry(comment_window)
        reservation_id_entry.pack(pady=5)

        # Validation button
        def validate_guest():
            guest_tc = guest_tc_entry.get().strip()
            reservation_id = reservation_id_entry.get().strip()

            if not guest_tc or not reservation_id:
                messagebox.showerror("Error", "Please fill out both fields")
                return

            # Validate guest TC and reservation ID
            reservation_info = fetch_reservation_details(reservation_id, guest_tc)  # Replace with the actual function
            if reservation_info:
                if reservation_info["is_canceled"]:
                    messagebox.showerror("Error", "Reservation is canceled and cannot be commented on.")
                else:
                    # Show comment entry form if validation passes
                    self.show_comment_form(comment_window, reservation_info)
            else:
                messagebox.showerror("Error", "Invalid Guest TC or Reservation ID")

        tk.Button(comment_window, text="Validate", command=validate_guest).pack(pady=10)
